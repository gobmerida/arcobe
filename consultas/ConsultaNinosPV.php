...
$cj_hijosSQL = "SELECT * FROM pv_planilla WHERE pv_añoperiodo='$anp' AND pv_destino='$tipo' ORDER BY pv_fechainscri";
...
if ($tipo == "CAMPAMENTO") {
  header('Content-Disposition: attachment; filename="PlanVacacional (Campamento Planta).xlsx"');
}
else {
  header('Content-Disposition: attachment; filename="PlanVacacional (Visita Guiada Planta).xlsx"');
}
