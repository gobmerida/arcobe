...
$insertar_ni = "INSERT INTO pv_planillace(id_nino,h_cedula,h_nombre1,h_nombre2,h_apellido1,h_apellido2,h_fecha_naci,h_sexo,cedula_mp,cedula_pm,cedula_regis,cedula_trb,h_gsanguineo,pv_planillanumero,id_periodo,pv_destino,pv_fotos,pv_certificado,pv_habilidades,pv_gustos,pv_vacunas,pv_alergias,pv_tratamiento,pv_alimentosp,pv_medicamentosp,pv_tchaqueta,pv_tfranela,pv_tmono,pv_tgorra,pv_contacto_cedula,pv_contacto_nombre,pv_contacto_apellido,pv_contacto_telefono,pv_contacto_parentesco,pv_observaciones,pv_edadmeses,codigo_trb)
				value('$id_nino','$h_cedula','$h_nombre1','$h_nombre2','$h_apellido1','$h_apellido2','$h_fecha_naci','$h_sexo','$cedula_mp','$cedula_pm','$cedula_regis','$cedula_regis','$h_gsanguineo','$pv_planillanumero','$id_pvperiodo','$pv_destino','$pv_fotos','$pv_certificado','$pv_habilidades','$pv_gustos','$pv_vacunas','$pv_alergias','$pv_tratamiento','$pv_alimentosp','$pv_medicamentosp','$pv_tchaqueta','$pv_tfranela','$pv_tmono','$pv_tgorra','$pv_contacto_cedula','$pv_contacto_nombre','$pv_contacto_apellido','$pv_contacto_telefono','$pv_contacto_parentesco','$pv_observaciones','$pv_edadmeses','$trb_c')";
...
if(array_key_exists('nombre_mp',$_POST) and array_key_exists("mp",$_POST)){
	$nombre_mp=$_POST['nombre_mp'];
	$apellido_mp=$_POST['apellido_mp'];
	mysql_query("INSERT INTO cj_mp(mp_cedula,mp_nombre,mp_apellido) value('$cedula_mp','$nombre_mp','$apellido_mp')",$con) or die (mysql_error());
}
if(array_key_exists('nombre_pm',$_POST) and array_key_exists("pm",$_POST)){
	$nombre_pm=$_POST['nombre_pm'];
	$apellido_pm=$_POST['apellido_pm'];
	mysql_query("INSERT INTO cj_mp(mp_cedula,mp_nombre,mp_apellido) value('$cedula_pm','$nombre_pm','$apellido_pm')",$con) or die (mysql_error());
}
